...
class Model_user extends CI_Model
{
    ...
    {
        if ($id) {
            $sql = "SELECT intUserID, vcUserName,vcPassword, vcFullName, vcEmail, vcContactNo, intUserGroupID, IsAdmin, intBranchID FROM member WHERE intUserID = ? AND IsActive = 1";
    ...
    public function removeUser($id)
    {
        if ($id) {
            ...
            $this->db->where('intUserID', $id);
            $delete = $this->db->update('member', $data);
